...
多中间件场景：依次调用对应 Skill 并综合结果。无法识别时提示用户指定中间件类型。

### 5.2 客户端生成组件

**模式**：模板 + 参数替换

- 每种中间件 × 语言组合对应一组模板（存 `references/`）
- 通过 paas-cli 动态获取环境连接信息
- 参数填入模板 → 生成代码 → 写入 target_path → 依赖提示

| 语言 | 生成文件 | 依赖管理 |
|------|---------|----------|
| Java | *Config.java, *Service.java, yml | Maven |
| Go | *_client.go, config.yaml | Go mod |
| Python | *_client.py, config.yaml | Pip |

**参数收集**：从上下文推断 → 主动询问 → 使用默认值（注明）
