#!/usr/bin/env python3
"""REDISSON-003: Check missing Redisson shutdown.

Detects RedissonClient without .shutdown() call.
"""
import re
import sys
...
REDISSON_CLIENT_PATTERN = re.compile(
    r'RedissonClient\s+\w+\s*=\s*Redisson\.create'
)
...
def check(project_root: str = ".") -> list:
    ...
    for path in sorted(root.rglob("*")):
        ...
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            if REDISSON_CLIENT_PATTERN.search(stripped):
        ...
        if not has_shutdown:
            for line_no, content in create_lines:
                violations.append({
                    ...
                    "line": line_no,
                    "message": f"REDISSON-003: RedissonClient without .shutdown() call: {content}",
                })
