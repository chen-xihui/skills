...
| 能力 | 说明 | 是否需要外部工具 | 支持的专项 Skill |
|------|------|----------------|----------------|
| 客户端创建与配置 | 自动生成客户端代码和配置文件 | 否 | Nacos、Redis、ES |
| 代码优化检查 | 扫描项目代码，按规则清单逐项检查 | 否 | Nacos、Redis、ES |
| 集群交互 | 通过 paas-cli 执行集群操作 | 是（paas-cli） | Nacos、Redis、ES |
| 故障排查 | 通过扁鹊平台和 paas-cli 诊断异常 | 是（扁鹊 + paas-cli） | Nacos、Redis、ES |
...
- v1.1.0 (2026-05-25): 更新能力概览，反映 Redis/ES 新增的服务接入指引能力；补充 Node.js 语言支持（ES）
